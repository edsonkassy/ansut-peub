...
{{-- Active le design system PEUB sur cette vue uniquement.
     ...
     NOTE DE REVUE : ce formulaire est le jumeau de parcours/edit.blade.php.
...
<div class="ds-container-tight ds-stack" style="padding-block: var(--space-4)">
    ...
    <header>
        ...
        <h1 style="margin-top: var(--space-1)">Ajouter un parcours</h1>
        <p class="ds-text-secondary" style="margin-top: var(--space-1)">Enregistrez votre parcours universitaire et vos formations</p>
    </header>
                ...
                <div style="display:grid; gap:var(--space-2); grid-template-columns:repeat(auto-fit, minmax(240px, 1fr))">
                    ...
                    <div>
                        ...
                        <select name="pays" id="pays" required class="ds-field @error('pays') ds-field-error @enderror">
                            @foreach($pays as $p)
                                <option value="{{ $p }}" @selected($p === "Côte d'Ivoire")>{{ $p }}</option>
                            @endforeach
                        </select>
                            ...
                            @foreach($niveaux as $group => $options)
                                <optgroup label="{{ $group }}">
                                    @foreach($options as $option)
                                        <option value="{{ $option }}" @selected(old('niveau') == $option)>{{ $option }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="annee_academique" id="annee_academique" required class="ds-field @error('annee_academique') ds-field-error @enderror">
                            ...
                            @foreach($annees as $annee)
                                <option value="{{ $annee }}" @selected(old('annee_academique') == $annee)>{{ $annee }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="mention" id="mention" required class="ds-field @error('mention') ds-field-error @enderror">
                            ...
                            @foreach($mentions as $mention)
                                <option value="{{ $mention }}" @selected(old('mention') == $mention)>{{ $mention }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="statut" id="statut" required class="ds-field @error('statut') ds-field-error @enderror">
                            @foreach($statuts as $statut)
                                <option value="{{ $statut }}" @selected(old('statut', 'en_cours') == $statut)>{{ ucfirst(str_replace('_', ' ', $statut)) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div style="grid-column:1 / -1">
                        ...
                        <input type="file" name="attestation_admission_file" id="attestation_admission_file" required
