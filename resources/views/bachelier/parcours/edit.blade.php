...
<div class="p-4 lg:p-8">
    <!-- Breadcrumb -->
    <x-breadcrumb text="PARCOURS / MODIFIER UN PARCOURS" />

    <div class="max-w-5xl mx-auto">
        <div class="bg-white rounded-xl shadow-lg p-6 lg:p-8 border border-gray-200">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Modifier le parcours</h1>
                <p class="text-gray-600 mt-1">Mettez à jour les informations de votre parcours</p>
            </div>
            
            <form action="{{ route('bachelier.parcours.update', $parcour) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="universite_nom" id="universite_nom" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" required value="{{ old('universite_nom', $parcour->universite_nom) }}">
                    ...
                    <div>
                        ...
                        <select name="pays" id="pays" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" required>
                            @foreach($pays as $p)
                                <option value="{{ $p }}" @if(old('pays', $parcour->pays) === $p) selected @endif>{{ $p }}</option>
                            @endforeach
                        </select>
                         @error('pays')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            @foreach($niveaux as $group => $options)
                                <optgroup label="{{ $group }}">
                                    @foreach($options as $option)
                                        <option value="{{ $option }}" {{ old('niveau', $parcour->niveau) == $option ? 'selected' : '' }}>{{ $option }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="annee_academique" id="annee_academique" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" required>
                            ...
                            @foreach($annees as $annee)
                                <option value="{{ $annee }}" {{ old('annee_academique', $parcour->annee_academique) == $annee ? 'selected' : '' }}>{{ $annee }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="performance" id="performance" placeholder="Ex: 15.50" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" value="{{ old('performance', $parcour->performance) }}" required min="0" max="20">
                        @error('performance')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select name="mention" id="mention" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" required>
                            ...
                            @foreach($mentions as $mention)
                                <option value="{{ $mention }}" {{ old('mention', $parcour->mention) == $mention ? 'selected' : '' }}>{{ $mention }}</option>
                            @endforeach
                        </select>
                        @error('mention')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                     ...
                     <div>
                        ...
                        <select name="statut" id="statut" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-[#00BFA5] focus:ring-1 focus:ring-[#00BFA5]" required>
                             @foreach($statuts as $statut)
                                <option value="{{ $statut }}" {{ old('statut', $parcour->statut) == $statut ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $statut)) }}</option>
                            @endforeach
                        </select>
                        @error('statut')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="file" name="attestation_admission_file" id="attestation_admission_file" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-[#00BFA5]/10 file:text-[#00BFA5] hover:file:bg-[#00BFA5]/20">
                        @if ($parcour->attestation_admission_file)
                            <p class="text-sm text-gray-500 mt-2">
                                Fichier actuel : 
                                <a href="{{ asset('storage/' . $parcour->attestation_admission_file) }}" target="_blank" class="text-[#00BFA5] hover:text-[#00BFA5]/80 font-medium">
