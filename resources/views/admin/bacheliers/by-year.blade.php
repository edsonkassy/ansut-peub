...
@section('title', 'Bacheliers ' . $year . ' - PEUB Admin')

@section('page-title', 'Bacheliers de l\'année ' . $year)
...
<div class="bg-white border border-gray-300 p-6 mb-6">
    <form method="GET" action="{{ route('admin.bacheliers.by-year', $year) }}">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            ...
            <div>
                ...
                <select name="status_profil" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                    <option value="">Tous</option>
                    <option value="verifie" {{ request('status_profil') === 'verifie' ? 'selected' : '' }}>Vérifié</option>
                    <option value="en_attente" {{ request('status_profil') === 'en_attente' ? 'selected' : '' }}>En Attente</option>
                    <option value="incomplet" {{ request('status_profil') === 'incomplet' ? 'selected' : '' }}>Incomplet</option>
                </select>
            </div>

            <!-- Boursier PEUB -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Boursier PEUB</label>
                <select name="boursier_peub" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                    <option value="">Tous</option>
                    <option value="1" {{ request('boursier_peub') === '1' ? 'selected' : '' }}>Oui</option>
                    <option value="0" {{ request('boursier_peub') === '0' ? 'selected' : '' }}>Non</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="region" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500">
            ...
            <div>
                ...
                <select name="serie_bac" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                    <option value="">Toutes</option>
                    @foreach($series as $key => $label)
                        <option value="{{ $key }}" {{ request('serie_bac') === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-md transition-colors">
                    <i data-lucide="search" class="w-4 h-4 inline mr-2"></i>
                    Filtrer
                </button>
                <a href="{{ route('admin.bacheliers.by-year', $year) }}" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-md transition-colors text-center">
        ...
        <tbody class="divide-y divide-gray-200">
            @forelse($bacheliers as $bachelier)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4">
                        <div class="text-sm font-medium text-gray-900">{{ $bachelier->nom }} {{ $bachelier->prenoms }}</div>
                        <div class="text-sm text-gray-500">{{ $bachelier->region }}</div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        @if($bachelier->score_final_peub !== null)
                            <div class="text-sm font-medium text-gray-900">{{ number_format($bachelier->score_final_peub, 2) }}/100</div>
                            @if($bachelier->rang_temporaire)
                                <div class="text-xs text-gray-500 mt-1">Rang #{{ $bachelier->rang_temporaire }}</div>
                            @endif
                        @else
                            <span class="text-sm text-gray-500">Non évalué</span>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        @if($bachelier->boursier_peub)
                            <span class="px-2 py-1 text-xs bg-secondary-100 text-secondary-700 rounded-full font-medium">
                                <i data-lucide="award" class="w-3 h-3 inline mr-1"></i>
                                Boursier
                            </span>
                        @else
                            <span class="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded-full font-medium">Non boursier</span>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="text-gray-500">
                            ...
                            <p class="text-sm">Aucun bachelier trouvé pour cette année.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
