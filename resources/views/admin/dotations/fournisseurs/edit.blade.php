...
@section('title', 'Modifier le Fournisseur - PEUB Admin')
...
<div class="bg-white border border-gray-300 mb-6 p-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-gray-600">Modifier les informations du fournisseur.</p>
        </div>
...
<div class="bg-white border border-gray-300 p-6">
    <form action="{{ route('admin.dotations.fournisseurs.update', $fournisseur) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type_fournisseur" id="type_fournisseur" class="w-full border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500 @error('type_fournisseur') border-red-500 @enderror">
                    ...
                    <option value="materiel_informatique" {{ old('type_fournisseur', $fournisseur->type_fournisseur) == 'materiel_informatique' ? 'selected' : '' }}>Matériel Informatique</option>
                    <option value="fournitures_scolaires" {{ old('type_fournisseur', $fournisseur->type_fournisseur) == 'fournitures_scolaires' ? 'selected' : '' }}>Fournitures Scolaires</option>
                    <option value="equipements_techniques" {{ old('type_fournisseur', $fournisseur->type_fournisseur) == 'equipements_techniques' ? 'selected' : '' }}>Équipements Techniques</option>
                    <option value="services" {{ old('type_fournisseur', $fournisseur->type_fournisseur) == 'services' ? 'selected' : '' }}>Services</option>
            ...
            <div>
                ...
                <select name="status" id="status" class="w-full border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500 @error('status') border-red-500 @enderror">
                    <option value="active" {{ old('status', $fournisseur->status) == 'active' ? 'selected' : '' }}>Actif</option>
                    <option value="suspendu" {{ old('status', $fournisseur->status) == 'suspendu' ? 'selected' : '' }}>Suspendu</option>
                    <option value="archive" {{ old('status', $fournisseur->status) == 'archive' ? 'selected' : '' }}>Archivé</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="contact_nom" id="contact_nom" value="{{ old('contact_nom', $fournisseur->contact_nom) }}"
            ...
            <div>
                ...
                <input type="email" name="contact_email" id="contact_email" value="{{ old('contact_email', $fournisseur->contact_email) }}"
            ...
            <div>
                ...
                <input type="tel" name="contact_telephone" id="contact_telephone" value="{{ old('contact_telephone', $fournisseur->contact_telephone) }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="adresse" id="adresse" rows="3" 
                          class="w-full border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500 @error('adresse') border-red-500 @enderror">{{ old('adresse', $fournisseur->adresse) }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="3" 
                          ...
                          placeholder="Informations supplémentaires sur le fournisseur...">{{ old('notes', $fournisseur->notes) }}</textarea>
            ...
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Contrat Actuel</label>
                <div class="flex items-center space-x-3 p-3 bg-gray-50 border border-gray-300">
            ...
            <!-- Nouveau contrat -->
            <div class="md:col-span-2">
                <label for="contrat" class="block text-sm font-medium text-gray-700 mb-1">
                    {{ $fournisseur->contrat_url ? 'Remplacer le contrat' : 'Ajouter un contrat' }} (optionnel)
                </label>
